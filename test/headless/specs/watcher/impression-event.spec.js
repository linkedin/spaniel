...
testModule(
  'Impression event',
  class extends ImpressionEventTestClass {
    ['@test should not fire if item is exposed but not impressed']() {
      return this.setupTest()
        .onDOMReady()
        .scrollTo(50)
        .waitForExposed()
        .assertOnce(ITEM_TO_OBSERVE, 'exposed', 'should be exposed')
        .assertNever(ITEM_TO_OBSERVE, 'impressed', 'should not be impressed')
        .done();
    }
